...
<div class="grid sm:grid-cols-2 gap-3">
    ...
    <div class="sm:col-span-2" x-show="storage !== 'upload'">
        <label class="block text-xs font-semibold text-ink-600 mb-1">URL o ID del vídeo</label>
        <input type="text" name="file_path" value="{{ $lesson && $lesson->storage !== 'upload' ? $lesson->file_path : '' }}" maxlength="2000" placeholder="https://www.youtube.com/watch?v=…"
               class="w-full rounded-lg border border-ink-200 px-3 py-2 text-sm focus:border-brand-400 outline-none">
    </div>
    ...
    <div class="sm:col-span-2">
        <label class="block text-xs font-semibold text-ink-600 mb-1">Descripción (opcional)</label>
        <textarea name="description" rows="2" class="w-full rounded-lg border border-ink-200 px-3 py-2 text-sm focus:border-brand-400 outline-none">{{ $lesson?->description }}</textarea>
    </div>
</div>
