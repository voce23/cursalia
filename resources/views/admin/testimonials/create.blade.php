...
@section('page-title', 'Nuevo testimonio')
...
<form method="POST" action="{{ route('admin.testimonials.store') }}" enctype="multipart/form-data" class="max-w-2xl space-y-5">
    @csrf
    @include('admin.partials._testimonial-fields', ['item' => null])
    <div class="flex items-center gap-2">
        <button type="submit" class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl font-bold bg-brand-600 text-white hover:bg-brand-700 shadow-lift transition">
            <i class="fa-solid fa-floppy-disk"></i> Crear testimonio
        </button>
