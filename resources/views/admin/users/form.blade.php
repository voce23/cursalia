...
<form method="POST" action="{{ route('admin.users.store') }}" class="max-w-xl space-y-5">
    @csrf
    ...
    <div class="bg-white border border-ink-200/70 rounded-3xl shadow-soft p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required maxlength="120" autofocus
    ...
    <div class="bg-white border border-ink-200/70 rounded-3xl shadow-soft p-6">
        ...
        <label class="block text-sm font-medium text-ink-700 mb-1.5">¿Qué será este usuario?</label>
        <select name="role" class="w-full px-4 py-3 rounded-2xl bg-cream-2 border border-ink-200 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:bg-white text-sm">
            @foreach (\App\Http\Controllers\Admin\UserController::CREATABLE_ROLES as $value => $label)
        ...
        <p class="mt-2 text-xs text-ink-500 leading-relaxed">
            <strong>Estudiante</strong>: aprende y se inscribe en cursos. <strong>Instructor</strong>: además puede impartir cursos (queda aprobado al crearlo aquí). <strong>Superadmin</strong>: acceso completo a este panel (se guarda como administrador).
        </p>
    </div>
    ...
    <div class="bg-white border border-ink-200/70 rounded-3xl shadow-soft p-6 space-y-4"
        ...
        <div>
            ...
            <div class="relative">
                <input :type="show ? 'text' : 'password'" name="password" x-model="pwd" required autocomplete="new-password"
    ...
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('admin.users.index') }}" class="px-5 py-3 rounded-2xl font-semibold bg-cream-2 text-ink-700 hover:bg-ink-100 transition">Cancelar</a>
    </div>
</form>
